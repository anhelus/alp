...
### Interagire con lo Stream Standard: `printf` e `scanf`
...
### Aprire e Chiudere un File: `fopen` e `fclose`
...
### Leggere e Scrivere su File: `fscanf` e `fprintf`
...
### Leggere un File fino alla Fine: Il Pattern Corretto

!!! warning "Il problema di `feof()`"
    Molti principianti leggono un file con `while (!feof(fp))`. **Questo è un errore**. `feof()` diventa vera solo **dopo** aver tentato di leggere oltre la fine del file, causando un'ultima iterazione con dati non validi.
...
1. Chiede all'utente nome, cognome ed età in modo sicuro.
