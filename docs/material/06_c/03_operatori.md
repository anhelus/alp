...
### 3.1 Regole Fondamentali: Precedenza e Associatività
...
### 3.2 Tipi di Operatori

*   **Operatore di Assegnazione (`=`)**: Assegna il valore dell'espressione a destra alla variabile a sinistra. Attenzione a non confonderlo con l'operatore di uguaglianza! Un errore comune è scrivere `if (a = 10)`: questa espressione assegna `10` ad `a` e risulta sempre vera, creando un bug. La forma corretta per un confronto è `if (a == 10)`.
...
### 3.3 Conversioni di Tipo (Casting)
