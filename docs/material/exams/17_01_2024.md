...
!!!note "Nota per il calcolo delle statistiche"
    Ovviamente, questo modo di procedere vale soltanto nel caso il vettore sia ordinato. In caso di vettore non ordinato, il massimo ed il minimo dovranno essere calcolati mediante due semplici cicli enumerativi.

### Parte 4: `stampa()`

L'ultima parte prevede la stampa a schermo del vettore. Per farlo, potremo scegliere di usare la funzione `fprintf` all'interno di un ciclo, oppure costruire una stringa e visualizzarla al termine. Scegliamo la seconda modalità:
...
!!!note "Utilizzo di disp"
    Si potrebbe anche pensare di usare opportunamente la funzione `disp`, tenendo però conto delle sequenze di escape.
